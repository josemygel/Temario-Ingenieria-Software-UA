...
class ReservaMockTest {
    ...
    void initState(){
        ...
        mockFactoria = partialMockBuilder(FactoriaBOs.class).addMockedMethod("getOperacionBO").createMock(imc);
        mockOperacion= imc.createMock(IOperacionBO.class);
    ...
    void realizaReservaC1() {
        ...
        expect(sut.compruebaPermisos(login,password,Usuario.BIBLIOTECARIO)).andReturn(false);
    ...
    void realizaReservaC2() {
        ...
        //Programamos expectativas
        expect(sut.compruebaPermisos(login,password,Usuario.BIBLIOTECARIO)).andReturn(true);

        //Inyectamos en la factoria
        assertDoesNotThrow(()->{
            //Haremos que el mock no devuelva ninguna excepcion con las llamadas
            mockOperacion.operacionReserva(socio, isbns[0]); expectLastCall().andVoid();
            mockOperacion.operacionReserva(socio, isbns[1]); expectLastCall().andVoid();
        });
        mockFactoria.op = mockOperacion;
...
//        assertEquals(esperado, reserva.getMessage());
    ...
    void realizaReservaC3() {
        ...
        assertDoesNotThrow(()->{
            //Haremos que el mock no devuelva ninguna excepcion con las llamadas
            mockOperacion.operacionReserva(socio, isbns[0]); expectLastCall().andThrow(new IsbnInvalidoException());
        });
        mockFactoria.op = mockOperacion;
    ...
    void realizaReservaC4() {
        ...
        assertDoesNotThrow(()->{
            //Haremos que el mock no devuelva ninguna excepcion con las llamadas
            mockOperacion.operacionReserva(socio, isbns[0]); expectLastCall().andThrow(new SocioInvalidoException());
        });
        mockFactoria.op = mockOperacion;
    ...
    void realizaReservaC5() {
        ...
        assertDoesNotThrow(()->{
            //Haremos que el mock no devuelva ninguna excepcion con las llamadas
            mockOperacion.operacionReserva(socio, isbns[0]); expectLastCall().andThrow(new JDBCException());
        });
        mockFactoria.op = mockOperacion;
