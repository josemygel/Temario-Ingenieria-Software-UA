...
func handler(w http.ResponseWriter, req *http.Request) {
	...
	case "register": // ** registro
		...
		u.Data["ddbb"] = req.Form.Get("ddbb") // clave privada
...
func client() {
	...
	keyLogin := keyClient[:32]  // una mitad para el login (256 bits) (+ salt luego)
	...
	// comprimimos, ciframos y codificamos la clave privada
	data.Set("ddbb", encode64(encrypt(compress([]byte("{DDBB}")), keyData)))
	...
	//	res.Msg = r.Request.Form.Get("ddbb")

	fmt.Println("BODY: ", res.Msg)
	dec := decode64(res.Msg)
	decry := decrypt(dec, keyData)
	decomp := decompress(decry)
	fmt.Println(string(decomp)) // mostramos el cuerpo de la respuesta (es un reader)

	res.Msg = string(decomp)
	json(res)
}
